...
export const extractCustomTag = (node: Node, tagName: string): SectionArg[] | undefined => {
 ...
 return nodes ? nodes.map((section: Node) => ({ section, match: section.value ? section.value.match(/(\w+)\s*=\s*((["'])(.*?)\3|([^>\s]*)(?=\s|\/>))(?=[^<]*>)/g): undefined }) )
      .map(({ section, match }: { section: Node; match?: string[] | null}) => ({
        attrs: { section, tagName, node },
        attributes: match ? match.map((m: string) => m.split('=').map(value => cleanQuotes(value))) : undefined,
      })) : undefined;
}

export const inlineNewContent = ({section, tagName, node}: AttrsArg, newContent: Node[]) => {
  ...
  if (node.children) {
    for (let i = index + 1; i < node.children.length; i +=1) {
      ...
      if (childNode && childNode.type == 'html' && (childNode.value == endTag || (childNode.value && childNode.value.toLowerCase().startsWith(`<${tagName}`)))) {
        break;
      }
    }
...
export const traverseDirs = (attributes:  string[][] | undefined, callback: TraverseCallback) => {

  const getDirectories = (folder: string, excludeFiles: string[], repoDir: string, newNodes: Node[]) => {
