...
type: [PageType](/tutorial/structure/configuration#pagetype)

Document type - blogs, pages, stories and even custom ones. By default all documents are type `story`
...
Whether to add a sidebar to navigate all the pages from the same page type. The default value is from the page type configuration.

### contextSidebar

type: `boolean`
  
Whether to add conext sidebar to navigate the sections of the page. The default value is from the page type configuration.

### fullPage

type: `boolean

Whether the page to take full width (pagecontainer.full theme variant to be customized). The default value is from the page type configuration.
