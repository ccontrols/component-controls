...
Our esm stories format is loosely compatible with storybook's [CSF](https://storybook.js.org/docs/formats/component-story-format/) with a few properties in common (ie title, component, decorators). However our esm stories format is fully extensible beyond just using a select few hard-coded props (ie parameters).
