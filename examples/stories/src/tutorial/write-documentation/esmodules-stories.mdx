...
Our esm stories format is loosely based on storybook's [CSF](https://storybook.js.org/docs/formats/component-story-format/) with a few properties in common (ie title, component, decorators) to keep basic compatibility. However our esm stories format is fully extensible beyond just using a select hard-coded props (ie parameters).
