...
The [controls api](/tutorial/structure/controls-api) provides an extensible interface for specifying and using input controls to enter component property values at run-time.

You can also check the list of [pre-defined controls](/tutorial/structure/controls) that are available for creating interactive examples of your components.
...
<Playground>
  <Story name="no-props-example" controls={{ color: { type: ControlTypes.COLOR, value: 'red' }, padding: { type: ControlTypes.NUMBER, value: 10 } }}>
    {() => (
...
2. Some named properties are passed to the example. In this case, only the properties specifically listed will be available at run-time (ie: `color`):

```jsx
export const overview = ({ color }) => <button style={{ color }}>some props</button>
```

<Playground>
  <Story name="named-props-example" controls={{ color: { type: ControlTypes.COLOR, value: 'red' }, padding: { type: ControlTypes.NUMBER, value: 10 } }}>
    {({ color }) => (
      <button style={{ color }}>some props</button>
    )}
  </Story>
</Playground>
...
<Playground>
  <Story name="all-props-example" controls={{ color: { type: ControlTypes.COLOR, value: 'red' }, padding: { type: ControlTypes.NUMBER, value: 10 } }}>
    {props => (
...
[data](/tutorial/structure/controls-api#componentcontroldata) helper information to generate random data will be used in conjunction with [faker.js](https://github.com/marak/Faker.js/).
