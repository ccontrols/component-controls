...
export const parseExports = (
  ...
  if (!cache.exports) {
    cache.exports = { named: {}, default: {} };
    traverse(cache.ast, traverseExports(cache.exports, imports));
  }
  return cache.exports;
};

export const traverseExports = (
  results: ExportTypes,
  imports: ImportTypes,
): TraverseOptions => {
      ...
      if (
        ...
        if ((rightExpression as ObjectExpression).properties) {
          ...
          const localName =
            ...
            ((rightExpression as MemberExpression).object as Identifier)?.name;
          const namedExport = localExports[localName] || imports[localName];
...
export const extractExports = (
  ...
  traverse(ast, traverseExports(results, imports));
  return results;
};
